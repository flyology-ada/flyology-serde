...
procedure Flyology_Serde_Generator.Build_Processes.Spawn_Abort_Test is
...
begin
   ...
   Require (Initialized, "spawn-abort budget did not initialize");
   ...
   Require (Build_State = Build_Succeeded, "spawn-abort command did not build");

   Test_Hooks.Arm_Spawn_Publish_Pause;
   Runner.Start;
   Test_Hooks.Wait_For_Spawn_Published (2.0, Reached);
   Require (Reached, "abort did not reach positive child publication");
   Test_Hooks.Last_Spawned_Child (Child);
   Require (Child > 0, "spawn pause did not retain the exact child identity");
   abort Runner;
   Test_Hooks.Release_Spawn_Pause;
   for Attempt in 1 .. 2_000 loop
      exit when Runner'Terminated;
      delay 0.001;
   end loop;
   Require (Runner'Terminated, "spawn-publication abort did not finish cleanup");
   for Attempt in 1 .. 2_000 loop
      exit when ABI.Kill (ABI.Process_ID (Child), 0) = -1
   ...
   Require
     (ABI.Kill (ABI.Process_ID (Child), 0) = -1
      and then ABI.Current_Errno = ABI.Errno_No_Process,
      "spawn-publication abort left its exact child alive");
end Flyology_Serde_Generator.Build_Processes.Spawn_Abort_Test;
