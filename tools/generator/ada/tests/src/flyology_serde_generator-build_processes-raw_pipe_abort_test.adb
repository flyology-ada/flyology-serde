...
procedure Flyology_Serde_Generator.Build_Processes.Raw_Pipe_Abort_Test is
...
begin
   ...
   Require (Initialized, "raw-pipe abort budget did not initialize");
   ...
   Require (Build_State = Build_Succeeded, "raw-pipe abort command did not build");

   Test_Hooks.Arm_Raw_Pipe_Pause;
   Runner.Start;
   Test_Hooks.Wait_For_Raw_Pipe_Open (2.0, Reached);
   Require (Reached, "raw-pipe abort did not reach the owned descriptor window");
   Test_Hooks.Last_Raw_Pipe (Raw_Read, Raw_Write);
   abort Runner;
   ...
   Require (Runner'Terminated, "raw-pipe abort did not finish cleanup");
   Require
     (Descriptor_Is_Closed (Interfaces.C.int (Raw_Read)) = 1
      and then Descriptor_Is_Closed (Interfaces.C.int (Raw_Write)) = 1,
      "raw-pipe abort leaked a descriptor outside its controlled owner");
end Flyology_Serde_Generator.Build_Processes.Raw_Pipe_Abort_Test;
